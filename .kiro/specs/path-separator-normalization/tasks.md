...
- [x] 3. Implement path separator normalization fix

  - [x] 3.1 Add `original_path` column to schema DDL
  ...
  - [x] 3.2 Add `original_path` field to Entry Pydantic model
  ...
  - [x] 3.3 Add `normalize_path` helper in `repository.py`
  ...
  - [x] 3.4 Normalize in `get_child_entries`
  ...
  - [x] 3.5 Normalize in `create_entries_bulk`
  ...
  - [x] 3.6 Normalize on CSV import in `csv_importer.py`
  ...
  - [x] 3.7 Normalize `path` parameter in `get_folder_summary` and fix subfolder filtering
  ...
  - [x] 3.8 Use `original_path` in export
  ...
  - [x] 3.9 Create migration script for existing data
